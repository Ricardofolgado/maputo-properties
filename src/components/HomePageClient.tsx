...
export default function HomePageClient({ featured }: { featured: Property[] }) {
    ...
    <>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-32 pb-40 md:pb-48">
          ...
          <div className="flex justify-center gap-8 sm:gap-16 mt-28 text-center animate-fade-in" style={{ animationDelay: "0.3s" }}>
            <div>
              ...
              <p className="text-white/60 text-sm mt-1">{t("hero.stats.properties", lang)}</p>
            </div>
            <div>
              <p className="text-2xl sm:text-3xl font-bold text-white">4</p>
              <p className="text-white/60 text-sm mt-1">{t("hero.stats.cities", lang)}</p>
            </div>
            <div>
              <p className="text-2xl sm:text-3xl font-bold text-white">4</p>
              <p className="text-white/60 text-sm mt-1">{t("hero.stats.types", lang)}</p>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="flex items-end justify-between mb-12">
          <div>
            <span className="text-primary font-semibold text-sm uppercase tracking-wider">{t("featured.badge", lang)}</span>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2">{t("featured.title", lang)}</h2>
            <p className="text-gray-500 mt-2 max-w-xl">{t("featured.subtitle", lang)}</p>
          </div>
          <a href="/listings" className="hidden sm:inline-flex items-center gap-2 text-primary font-semibold hover:gap-3 transition-all bg-primary-light px-5 py-2.5 rounded-full text-sm">
        ...
        {featured.length > 0 ? (
          ...
          <div className="text-center py-20 bg-gradient-to-br from-gray-50 to-gray-100 rounded-3xl border border-gray-100">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">{t("featured.empty.title", lang)}</h3>
            <p className="text-gray-400 mb-6 max-w-md mx-auto">{t("featured.empty.desc", lang)}</p>
            <a href="/cadastrar" className="inline-flex bg-primary text-white px-8 py-3 rounded-full font-semibold hover:bg-primary-dark transition-all hover:shadow-lg hover:shadow-primary/25 active:scale-95">
              {t("featured.empty.cta", lang)}
            </a>
          </div>
        )}

        <div className="text-center mt-10 sm:hidden">
          <a href="/listings" className="inline-flex items-center gap-2 text-primary font-semibold bg-primary-light px-6 py-3 rounded-full">
            {t("featured.viewAll", lang)} {t("nav.imoveis", lang)}
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
            </svg>
          </a>
      ...
      <section className="bg-gradient-to-b from-gray-50 to-white py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-primary font-semibold text-sm uppercase tracking-wider">{t("featured.badge", lang)}</span>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mt-2 mb-4">{t("why.title", lang)}</h2>
            <p className="text-gray-500 max-w-2xl mx-auto">{t("why.subtitle", lang)}</p>
          </div>
            ...
            {[
              {
                ...
                title: t("why.search.title", lang),
                desc: t("why.search.desc", lang),
              ...
              },
              {
                ...
                title: t("why.contact.title", lang),
                desc: t("why.contact.desc", lang),
                gradient: "from-accent-gold to-stone-400",
              },
              {
                icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />,
                title: t("why.verified.title", lang),
                desc: t("why.verified.desc", lang),
                gradient: "from-accent-teal to-primary",
              },
            ].map((item, i) => (
              <div key={i} className="group relative bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative bg-gradient-to-br from-primary via-primary-dark to-stone-900 rounded-3xl overflow-hidden">
            ...
            <div className="relative px-8 py-16 md:py-20 md:px-16 text-center">
              <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{t("cta.title", lang)}</h2>
              <p className="text-white/70 text-lg mb-8 max-w-xl mx-auto">{t("cta.desc", lang)}</p>
              <a href="/cadastrar" className="inline-flex items-center gap-2 bg-white text-primary font-bold px-8 py-3.5 rounded-full hover:bg-gray-50 transition-all hover:shadow-2xl active:scale-95">
                {t("cta.btn", lang)}
